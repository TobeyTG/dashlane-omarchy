...
Rectangle {
  ...
  RowLayout { anchors.fill: parent; anchors.leftMargin: 14; anchors.rightMargin: 10; spacing: 12
    Rectangle { width: 36; height: 36; radius: 8; color: Theme.bg2
      Text { anchors.centerIn: parent; color: Theme.accent; font.family: Theme.font; font.pixelSize: 15; font.bold: true; textFormat: Text.PlainText; text: Vault.initial(root.entry) } }
    ColumnLayout { Layout.fillWidth: true; spacing: 2
      Text { textFormat: Text.PlainText; text: Vault.name(root.entry); color: Theme.fg; font.family: Theme.font; font.pixelSize: 14; elide: Text.ElideRight; Layout.fillWidth: true }
      Text { textFormat: Text.PlainText; text: [root.entry.login || root.entry.email, Vault.host(root.entry)].filter(Boolean).join("  ·  "); color: Theme.fg2; font.family: Theme.font; font.pixelSize: 12; elide: Text.ElideRight; Layout.fillWidth: true }
    }
